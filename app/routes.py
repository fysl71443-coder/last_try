...
from models import MenuCategory, MenuItem, SalesInvoice, SalesInvoiceItem, Customer, PurchaseInvoice, PurchaseInvoiceItem, ExpenseInvoice, ExpenseInvoiceItem, Settings, Meal, MealIngredient, RawMaterial, Supplier, Employee, Salary, Payment, EmployeeSalaryDefault, TableSettings
...
def edit_employee(emp_id):
    ...
    if request.method == 'POST':
        ...
        # active checkbox
        try:
            emp.active = True if str(request.form.get('active', emp.active)).lower() in ['1','true','yes','on'] else False
        except Exception:
            pass
        # work hours
        try:
...
def api_salaries_statements():
    ...
    try:
        ...
        return jsonify({'ok': False, 'error': str(e)}), 400
    return jsonify({'ok': True, 'year': year, 'month': month, 'rows': rows, 'totals': totals}), 200
...
def reports_print_salaries():
    ...
    meta = get_saudi_now().strftime('%Y-%m-%d %H:%M')
...
# Basic categories API for front-end use
@main.route('/api/categories', methods=['GET'], endpoint='api_categories')
@login_required
def api_categories():
    try:
        warmup_db_once()
        ...
        try:
            ...
            try:
                ...
                cats = MenuCategory.query.all()
        items = [{'id': c.id, 'name': c.name, 'sort_order': getattr(c, 'sort_order', 0)} for c in cats]
        return jsonify({'success': True, 'categories': items})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 400

# Load draft order as JSON (branch + table)
@main.route('/api/load-draft-order/<branch_code>/<int:table_number>', methods=['GET'], endpoint='api_draft_load')
@login_required
def api_draft_load(branch_code, table_number):
    if not user_can('sales','view', branch_code):
        return jsonify({'success': False, 'error': 'forbidden'}), 403
    try:
        draft = kv_get(f'draft:{branch_code}:{table_number}', {}) or {}
...
# Product details API (MenuItem first; fallback to Meal)
@main.route('/api/products/<int:item_id>', methods=['GET'], endpoint='api_product_get')
@login_required
def api_product_get(item_id):
    try:
        warmup_db_once()
        ...
        m = Meal.query.get(item_id)
        if m:
            return jsonify({'success': True, 'product': {
                'id': m.id,
                'name': m.name,
                'price': float(m.sale_price if getattr(m, 'sale_price', None) is not None else 0.0),
...
def vat_print():
    ...
    try:
        year = int(request.args.get('year') or 0) or get_saudi_now().year
    except Exception:
        year = get_saudi_now().year
    try:
        quarter = int(request.args.get('quarter') or 0) or ((get_saudi_now().month - 1)//3 + 1)
    except Exception:
        quarter = ((get_saudi_now().month - 1)//3 + 1)
